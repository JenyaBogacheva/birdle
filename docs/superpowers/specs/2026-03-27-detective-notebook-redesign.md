...
- Background stays on landing bird (or transitions to neutral blurred nature photo)
...
# Emits SSE: {"type": "candidates", "data": [...]}
# Frontend fetches Macaulay photos for new species_codes
```

These tools perform no external API calls — they relay data to the frontend via SSE events.
...
|---|---|---|---|
| Data tools | `get_regional_birds`, `web_search`, `get_species_image` | 8 calls | External APIs, cost and latency |
...
- **Landing bird:** Hardcoded Macaulay URL, preloaded on page load via `<link rel="preload">`
- **Candidate birds:** Fetched on-demand as `candidates` SSE events arrive. Frontend calls Macaulay Library for each new `species_code`.
- **Result bird:** Already fetched by backend (existing behavior — streaming endpoint fetches images before sending `result` event). Also loaded as candidate during thinking.
